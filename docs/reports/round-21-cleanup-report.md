...
| 批次 | 狀態 |
|---|---|
| **A2 × 20 seed(第 1–100 關)** | ✅ 全部跑完 —— Gate 4b / **5a** / 7 定版讀數(D6) |
| **A3 × 20 seed** | ✅ 全部跑完 —— Gate **5b** / 6a / 7 定版讀數(D7) |
| A1 / A0 / A4 × 20 seed | 🔄 跑緊(Gate 2a / 2b / 3a / 4a / 6b) |
| frozen(Gate 3b)/ contract(Gate 8) | ⏳ 排隊 |

即係話:**本輪要答嘅兩條 gate(5a 同 5b)已經係定版讀數 —— 40 個完整
campaign、4000 個關卡樣本。** 淨返嗰啲係 A1 / A0 / A4 嘅前段同天花板 gate
(佢哋唔關 boss 上限事)同兩個 mode。
...
| **5a A2 71–99** | **≤12%**(本輪重釘) | **19.3%** | **FAIL —— 診斷見 D5/D6,冇自行調** |
| **5b A3 71–99** | ≥28% | **58.4%** | **PASS** |
| 6a A3 第 100 關 | ≤5% | 0.0%(n=20) | PASS |
| 7 A2 段內回升 | ≤8 點 | +7.5 @ 31–40 | PASS |
| 7 A3 段內回升 | ≤8 點 | +7.5 @ 31–40 | PASS(舊切法係 +36.1 FAIL) |
| 1 / 1b(純算術) | — | `GATE1 PASS` / `GATE1B PASS` | PASS(套裝入面) |
...
**點樣繼續:雙擊 `tools\續跑定版job.bat`。** 佢會由斷點接落去 —— 已經有
`GATE DONE` 嘅單位跳過,跑到一半嗰啲整個刪走重跑。可以隨時閂窗、隨時再開。
跑完之後**雙擊 `tools\定版job出報告.bat`** 睇 Gate 表(未跑完都出得,但嗰陣
啲數係部分樣本,唔可以當定版)。

原始輸出全部喺 **`qa/bench/gate/r21/`**,一個 seed 一個檔。
A/B 對照組(關咗 boss 上限)喺 **`qa/bench/gate/r21_nofloor/`**。
...
| BOSS 開場上限 | `BOSSFLOOR PASS fails=0 (22 項)`,最短 boss 戰量到 6.00 秒 | `test/BossFloorTest.tscn` |
...
| 定版 Gate 7(新完成帶) | A2 +7.5 PASS / A3 +7.5 PASS(舊切法 A3 係 +36.1 FAIL) | 同上 |
...
| **`BossFloorTest`**(新) | `PASS fails=0 (22 項)` | boss 開場上限守門 |
...
**Gate 5a(A2 71–99 ≤12%)實測 19.3%,冇達標。** boss 開場上限做到咗佢承諾
...
* **雙擊 `tools\續跑定版job.bat`** —— 由斷點接落去,隨時可以閂窗。
  A2 同 A3 各二十個 seed 已經齊(即係本輪兩條主 gate 已經定版),
  淨返 A1 / A0 / A4 同 frozen / contract 兩個 mode。
