...
美術畫廊嗰粒「(除錯)」掣喺出街版嘅主選單上面,一撳就入,而佢峰值
**+5.03 MB / +496 nodes**。喺一部已經貼住上限嘅 iPhone 上面,呢個數唔細。
割走佢(或者喺 web 版收起)係一個**介面**決定,唔係一個 bug —— 所以留返
畀你決定,而唔係我順手改咗。
...
* **`scenes/Gallery.tscn`(美術畫廊,除錯用)仲喺 web pack 入面**,因為主選單
  有一粒掣連住佢。佢連腳本合共約 4 KB,所以割唔割佢係一個**介面**決定
  (要唔要喺出街版顯示一粒「除錯」掣),唔係一個大細決定 —— 我冇喺呢一輪
  自己做呢個決定。
