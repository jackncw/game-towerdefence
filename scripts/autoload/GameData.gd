...
#         單體輸出追唔到數量,要範圍
#   18 甲蟲皇 reflect
#      -> 屍體數 x 減傷:史萊姆(分裂)出屍體,幽靈(魔抗 25)同岩石巨像(護甲 12)
#         令嗰啲屍體唔係打得爛就得
#         第一版試過將呢關做成純「傷害類型」軸,量到嗰條軸單獨扛唔起一幅牆:
#         護甲 12 = 減傷 19%(a/(a+50)),魔抗 25 = 減傷 29%(m/(m+60)),而屍體數
#         軸係返生 1.45x 加分裂連鎖,有效血量 2-3 倍 —— 差成個數量級。三個減傷家族
#         一齊落、spawn_min 去到 0.06(即係每秒 16 隻)都仲係 100% 首通、零隻到門口。
#         所以減傷唔係一條獨立嘅軸,佢係一個乘數:要有屍體先值得減傷。呢一關教嘅
#         嘢因此係「你嘅範圍傷害而家要打得穿甲同魔抗」,同第 13 關嘅純範圍唔同。
#
...
var WALLS := {
	...
	18: {"pool": ["slime", "ghost", "golem"], "spawn_min": 0.07, "hint": "WALL_HINT_18"},
}
