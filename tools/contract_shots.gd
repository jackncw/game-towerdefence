...
## 合約關 UI 嘅截圖自查(第十五輪)。
##
## 影五張:選關(合約關同最終關嘅標記)、三選一卡片、揀完之後嘅 HUD badge、
## 撳開嘅「當前合約狀態」面板、第 100 關嘅十 boss 場面。
##
...
var OUTDIR := "res://qa/screenshots/round-15-contract/"
var sub: SubViewport
var done := false
...
func _run() -> void:
	...
	for i in 4:
		await get_tree().process_frame
	await _grab("03_after_pick_hud")
	b.hud._toggle_contract_summary()
	for i in 3:
		await get_tree().process_frame
	await _grab("04_contract_status")
	b.hud.hide_contract()
	# 再揀多兩張,睇疊加狀態同封頂顯示
	for k in 2:
		b.contract_pending = false
		b._open_contract_offer()
		for i in 2:
			await get_tree().process_frame
		if not b.contract_offer.is_empty():
			b.choose_contract(int(b.contract_offer[b.contract_offer.size() - 1]))
		for i in 2:
			await get_tree().process_frame
	b.hud._toggle_contract_summary()
	for i in 3:
