...
第 21 輪兩處重釘:
  ...
  * **Gate 7 段界 91 -> 81**(見下面 G7_SEGS 嘅註)。
...
print()
print('=== Gate7 難度分段單調 ===')
print('    段 = 1-10 / 11-40 / 41-70 / 71-80 / 81-99,段交界 41、71、81 容許回升。')
print('    平滑窗 = 10 關(一個完整家族/boss 輪轉週期)。合約關唔入序列。')
# 第 21 輪:完成帶由 91 移到 81。段界嘅意思一路都係「雙 tier-3 完成帶」,
# 而 TIER_JUMP 1.70 -> 1.95 之後 A3 課完第二件 tier-3 嘅時點提早咗一個 block
# (第十七輪實測 85-95),所以 91 呢條縫已經**遲過**真實完成點 —— 縫擺喺
# 完成點之後,回升就會落喺 81-90 呢個 block 入面被當成段內回升而肥佬,
# 而佢其實同 41(tier-2 入口)/ 71(tier-3 入口)完全同一性質。
G7_SEGS = [(1, 10), (11, 40), (41, 70), (71, 80), (81, 99)]
for a in arches:
    series = {}
