...
# ============================================================================
# TOWERS —— 程序生成嘅塔 sprite 已經【封存】(2026-08-07,塔 / 魔法美術收官輪)
# ============================================================================
# 60 張塔圖而家由 `tools/tower_cutout.py` 喺 `art_reference/tower/` 嘅 20 張
# sprite sheet 度摳出嚟。舊嗰套 Pillow 畫法(round-5 重畫 + round-10 進化階:
# 20 個 t_* 畫函數、tower_pad、_sig_towers、_tier_foundation / _tier_flourish、
# gen_towers)完整搬咗去 `tools/deprecated/gen_art_towers_v1.py`。
#
# `main()` 唔再叫 `gen_towers()` —— **唔好加返**,一行就會靜靜雞冚走成套新
# 美術(同 round-19 怪物嗰單一模一樣)。守門嘅係 `test/TowerArtTest`。
#
# 兩個 helper 冇跟住走,因為佢哋唔係塔專用:
#   `_tglow`      —— 魔晶 / 傳送門 / 特效圖都用緊
#   `_tier_lift`  —— 魔法 icon 嘅階級調色仲用緊
# ============================================================================
...
def gen_spells():
    ...
    for i, fn in enumerate(SPELLS, 1):
        for tier in (1, 2, 3):
            if (i, tier) not in PROCEDURAL_SPELLS:
                continue
            c = Canvas(264)
            fn(c)
            if tier == 1:
                # frame drawn edge-to-edge, no silhouette outline needed
                save(c.img.resize((44, 44), Image.NEAREST), "spells",
                     f"spell_{i}.png")
            else:
                _spell_tier_frame(c, tier)
                save(_tier_lift(c.img, tier).resize((44, 44), Image.NEAREST),
                     "spells", f"spell_{i}_t{tier}.png")
            n += 1
    return n
...
def main():
    ...
    # NOTE: 冇 counts["towers"] —— 塔圖 2026-08-07 之後由 tools/tower_cutout.py
    # 出(見上面 TOWERS 段)。魔法 icon 淨返 PROCEDURAL_SPELLS 嗰四格。
